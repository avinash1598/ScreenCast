 ...
 * Created by AVINASH on 9/11/2016.
 */
...
public class CastService extends Service {
    private final String TAG = "com.example.avinash.screencast2.CastService";
    private final int NT_ID_CASTING = 0;
    private Handler handler = new Handler(new ServiceHandlerCallback());
    ...
    public void onCreate() {
        super.onCreate();
        Log.d("service started", "yo00");
        mMediaProjectionManager = (MediaProjectionManager) getSystemService(Context.MEDIA_PROJECTION_SERVICE);
        broadcastIntentFilter = new IntentFilter();
        broadcastIntentFilter.addAction(Common.ACTION_STOP_CAST);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        Log.d("service started","yo");
        if (intent == null) {
            return START_NOT_STICKY;
        }
        mReceiverIp = intent.getStringExtra(Common.EXTRA_RECEIVER_IP);
        resultCode = intent.getIntExtra(Common.EXTRA_RESULT_CODE, -1);
        resultData = intent.getParcelableExtra(Common.EXTRA_RESULT_DATA);
        ...
        //    return START_NOT_STICKY;
        //}
        Width = intent.getIntExtra(Common.EXTRA_SCREEN_WIDTH, Common.DEFAULT_SCREEN_WIDTH);
        Height = intent.getIntExtra(Common.EXTRA_SCREEN_HEIGHT, Common.DEFAULT_SCREEN_HEIGHT);
        Dpi = intent.getIntExtra(Common.EXTRA_SCREEN_DPI, Common.DEFAULT_SCREEN_DPI);
        Bitrate = intent.getIntExtra(Common.EXTRA_VIDEO_BITRATE, Common.DEFAULT_VIDEO_BITRATE);
        Format = intent.getStringExtra(Common.EXTRA_VIDEO_FORMAT);
        if (Format == null) {Format = Common.DEFAULT_VIDEO_MIME_TYPE;
        }
    ...
    private void showNotification() {
        ...
        builder.setSmallIcon(R.mipmap.ic_launcher)
                ...
                .setContentText(("Screen Casting"))
                .addAction(android.R.drawable.ic_menu_close_clear_cancel, "Stopped", notificationPendingIntent);
    ...
    private boolean startScreenCapture() {
        Log.d(TAG, "mResultCode: " + resultCode + ", mResultData: " + resultData);
    ...
    private void startRecording() {
        ...
        mVirtualDisplay = mMediaProjection.createVirtualDisplay("Recording Display", Width,
                Height, Dpi, 0 /* flags */, inputSurface,
    ...
    private void prepareVideoEncoder() {
        ...
        //format.createAudioFormat(MediaFormat.);
        ...
        try {
            mVideoEncoder = MediaCodec.createEncoderByType(Format);
            mVideoEncoder.configure(format, null, null, MediaCodec.CONFIGURE_FLAG_ENCODE);
            inputSurface = mVideoEncoder.createInputSurface();
            mVideoEncoder.start();
        } catch (IOException e) {
            Log.d(TAG, "Failed to initial encoder, e: " + e);
            releaseEncoders();
        }
    }

    private boolean drainEncoder() {
        mDrainHandler.removeCallbacks(drainEncoderRunnable);
        while (true) {
        ...
        mDrainHandler.postDelayed(drainEncoderRunnable, 10);
        return true;
    }
    ...
    private boolean createServerSocket() {
        Thread th = new Thread(new Runnable() {
            @Override
            public void run() {
                try {Log.d("inside","createserversocket");
        ...
        public void run() {
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    ...
                    osw.write(String.format(HTTP_MESSAGE_TEMPLATE, Width,Height));
                    osw.flush();
                    mSocketOutputStream.flush();
                    if (Format.equals(MediaFormat.MIMETYPE_VIDEO_AVC)) {                /**sps and pps*/
                        if (Width == 1280 && Height == 720) {
                            ...
                            Log.e(TAG, "Unknown width: " + Width + ", height: " + Height);
                    ...
                    if (mSocketOutputStream != null) {
                        handler.post(mStartEncodingRunnable);
                    }
                    return;
    ...
    private boolean createSocket() {                    /////////////////////required by us
        Thread th = new Thread(new Runnable() {
            @Override
            public void run() {
                try {
                    Log.d("inside","createsocket");
                    ...
                    mSocketOutputStream.flush();
                    if (Format.equals(MediaFormat.MIMETYPE_VIDEO_AVC)) {
                        if (Width == 1280 &&Height == 720) {
                            mSocketOutputStream.write(H264_PREDEFINED_HEADER_1280x720);
                        } else if (Width == 800 && Height == 480) {
                            mSocketOutputStream.write(H264_PREDEFINED_HEADER_800x480);
